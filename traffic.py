...
def load_data(data_dir):
    ...
    categories = [category for category in os.listdir(directory)]
    data = ([], [])
    for category in categories:
        print(f"Extract images for category: {category}")
        category_path = os.path.join(directory, category)
        images = [image for image in os.listdir(category_path)]
        for index, image in enumerate(images):
            data[1].append(int(category))
            image_path = os.path.join(category_path, image)
            numpy_image = cv2.imread(image_path)
            resized_image = cv2.resize(numpy_image, (IMG_WIDTH, IMG_HEIGHT))
            resized_image = resized_image / 255
            data[0].append(resized_image)
    return data
